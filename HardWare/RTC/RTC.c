...
#define RTC_CLOCK_SOURCE_IRC32K		// 配置RTC时钟源（使用内部32K，避免外部晶振未焊接时卡死）
 ...
 void rtc_setup(void)
  {
      ...
      rtc_initpara.year = 0x25;           // 2025年 (BCD)
      rtc_initpara.day_of_week = RTC_SATURDAY;
      rtc_initpara.month = RTC_MAY;       // 五月
      rtc_initpara.date = 0x18;           // 18日 (BCD)
      rtc_initpara.display_format = RTC_24HOUR;
      rtc_initpara.am_pm = RTC_AM;
      rtc_initpara.hour = 0x10;           // 10时 (BCD)
      rtc_initpara.minute = 0x30;         // 30分 (BCD)
      rtc_initpara.second = 0x00;         // 00秒 (BCD)
